{
  ...
  nodes.machine = {pkgs, ...}: {
    ...
    environment = {
      ...
      etc = {
        ...
        # mcp.devenv server). The cwd (/home/alice) has no devenv.nix, so
        # a successful initialize + tools/list PROVES the wrapper fell
        # back to the pinned ~/.config/devenv-agent project and the
        # server is up (its cache init runs in the background; the
  ...
  testScript = ''
    ...
    # 2c+. Verify devenv: the flake-package CLI on PATH, the local devenv
    # MCP entry with the wrapper command, and the pinned fallback project
    # generated by the devenv home module + opencode module.
    machine.succeed("su - alice -c 'devenv --version'")
    machine.succeed("su - alice -c 'jq -e \".mcp.devenv.type == \\\"local\\\"\" ~/.config/opencode/opencode.json'")
    machine.succeed("su - alice -c 'jq -e .mcp.devenv.command ~/.config/opencode/opencode.json'")
