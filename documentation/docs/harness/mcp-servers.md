...
| `mcp.devenv.enable` | `false` | `mcp.devenv` | local | devenv MCP server — local stdio `devenv mcp` via the `devenv-mcp-opencode` wrapper: `search_packages` (packages in the project's `devenv.yaml` nixpkgs input) and `search_options` (devenv module options). devenv hard-exits outside devenv projects, so the wrapper mirrors devenv's own discovery (walks up for `devenv.nix`) and falls back to the pinned `~/.config/devenv-agent` project generated by the devenv home module (`nixpkgs` = the flake's `nixpkgs` input as a store path, no runtime fetch). The devenv CLI comes from the `devenv` flake input (pinned `v2.3.1`) routed through the overlay as `pkgs.devenv`; that home module (`homeSpec.programs.devenv.enabled` / `.autoActivate.enabled`) also installs devenv on PATH and wires the devenv 2.x native auto-activation bash hook. Enabled for the dev profile |
...
| `mcp.varlock-docs.enable` | `false` | `mcp.varlock-docs` | remote | Varlock docs MCP server (`https://docs.mcp.varlock.dev/mcp`) — searches the varlock.dev documentation; public, no auth needed |

The opt-in servers (cloudflare ×6, mdn, artifacthub, kubernetes, typeui,
devenv, varlock-docs) are enabled for the **dev profile** — see
